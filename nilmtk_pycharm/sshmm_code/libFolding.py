    ...
    def __init__(self, data, folds=10, reverse=False, verbose=True):
        ...
        if self.folds == 1:
            ...
            if len(self.data) > self.folds:
                self.data[-2] = pandas.concat(self.data[-2:]) # concat only extra last element with immediate ast one and replace the the last but one with the combine one
                del self.data[-1] # now delete this extra one last element
